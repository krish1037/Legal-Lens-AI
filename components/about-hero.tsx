"use client"

import { motion } from "framer-motion"
import { Scale, Users, Globe, Award, Brain, Shield, Target, Clock } from "lucide-react"

const stats = [
  {
    icon: Users,
    value: "1K+",
    label: "Legal Professionals",
    description: "Trust our AI analysis",
    color: "blue",
  },
  {
    icon: Globe,
    value: "20+",
    label: "States of India",
    description: "Using Legal Lens AI",
    color: "green",
  },
  {
    icon: Scale,
    value: "10k+",
    label: "Documents",
    description: "Successfully analyzed",
    color: "purple",
  },
  {
    icon: Award,
    value: "90.2%",
    label: "Accuracy Rate",
    description: "In legal analysis",
    color: "orange",
  },
]

export function AboutHero() {
  return (
    <section className="relative min-h-screen px-4 sm:px-6 lg:px-8 py-20 overflow-hidden">
      <div className="max-w-7xl mx-auto">
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          {/* Left Side - Main Content */}
          <div className="space-y-8">
            <motion.div initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }}>
              <div className="inline-flex items-center px-4 py-2 bg-primary/10 rounded-full text-primary text-sm font-medium mb-6">
                <Brain className="h-4 w-4 mr-2" />
                About Legal Lens AI
              </div>

              <h1 className="text-5xl lg:text-6xl font-bold text-foreground mb-6 text-balance leading-tight">
                Revolutionizing
                <br />
                <span className="bg-gradient-to-r from-primary to-secondary bg-clip-text text-transparent">
                  Legal Analysis
                </span>
                <br />
                Through AI
              </h1>

              <p className="text-xl text-muted-foreground mb-8 text-pretty max-w-lg">
                Making legal information accessible through cutting-edge AI technology. We believe everyone deserves to
                understand their legal documents with <span className="text-primary font-semibold">90.2% accuracy</span>.
              </p>

              {/* Feature Badges */}
              <div className="flex flex-wrap gap-3 mb-8">
                <div className="flex items-center px-4 py-2 bg-blue-100 text-blue-800 rounded-full text-sm font-medium">
                  <Shield className="h-4 w-4 mr-2" />
                  Secure & Private
                </div>
                <div className="flex items-center px-4 py-2 bg-purple-100 text-purple-800 rounded-full text-sm font-medium">
                  <Brain className="h-4 w-4 mr-2" />
                  AI-Powered
                </div>
                <div className="flex items-center px-4 py-2 bg-green-100 text-green-800 rounded-full text-sm font-medium">
                  <Target className="h-4 w-4 mr-2" />
                  90.2% Accurate
                </div>
              </div>
            </motion.div>

            {/* Bottom Stats Grid */}
            <motion.div
              initial={{ opacity: 0, y: 30 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.8, delay: 0.4 }}
              className="grid grid-cols-2 gap-4"
            >
              <div className="glass p-6 rounded-2xl bg-gradient-to-br from-blue-50 to-blue-100 border border-blue-200">
                <div className="flex items-center mb-3">
                  <Users className="h-6 w-6 text-blue-600 mr-3" />
                  <span className="text-2xl font-bold text-blue-900">1K+</span>
                </div>
                <p className="text-blue-700 font-medium">Legal Professionals use our platform</p>
              </div>

              <div className="glass p-6 rounded-2xl bg-gradient-to-br from-green-50 to-green-100 border border-green-200">
                <div className="flex items-center mb-3">
                  <Globe className="h-6 w-6 text-green-600 mr-3" />
                  <span className="text-2xl font-bold text-green-900">22+</span>
                </div>
                <p className="text-green-700 font-medium">Availability in States of India</p>
              </div>

              <div className="glass p-6 rounded-2xl bg-gradient-to-br from-purple-50 to-purple-100 border border-purple-200">
                <div className="flex items-center mb-3">
                  <Scale className="h-6 w-6 text-purple-600 mr-3" />
                  <span className="text-2xl font-bold text-purple-900">10K+</span>
                </div>
                <p className="text-purple-700 font-medium">Documents Analyzed</p>
              </div>

              <div className="glass p-6 rounded-2xl bg-gradient-to-br from-orange-50 to-orange-100 border border-orange-200">
                <div className="flex items-center mb-3">
                  <Award className="h-6 w-6 text-orange-600 mr-3" />
                  <span className="text-2xl font-bold text-orange-900">90.2%</span>
                </div>
                <p className="text-orange-700 font-medium">Accuracy Rate</p>
              </div>
            </motion.div>
          </div>

          {/* Right Side - Stats Grid */}
          <motion.div
            initial={{ opacity: 0, x: 50 }}
            animate={{ opacity: 1, x: 0 }}
            transition={{ duration: 0.8, delay: 0.2 }}
            className="grid grid-cols-2 gap-6"
          >
            {/* Mission Card */}
            <div className="glass p-8 rounded-3xl bg-gradient-to-br from-blue-50 to-emerald-100 border border-blue-200 hover:shadow-lg transition-all duration-300">
              <div className="flex items-center justify-center w-16 h-16 bg-blue-500 rounded-2xl mb-6 mx-auto">
                <Brain className="h-8 w-8 text-white" />
              </div>
              <div className="text-center">
                <div className="text-4xl font-bold text-blue-900 mb-2">Empowering</div>
                <p className="text-blue-700 font-medium">India about their law and rights</p>
              </div>
            </div>

            {/* Vision Card */}
            <div className="glass p-8 rounded-3xl bg-gradient-to-br from-purple-50 to-violet-100 border border-purple-200 hover:shadow-lg transition-all duration-300">
              <div className="flex items-center justify-center w-16 h-16 bg-purple-500 rounded-2xl mb-6 mx-auto">
                <Target className="h-8 w-8 text-white" />
              </div>
              <div className="text-center">
                <div className="text-4xl font-bold text-purple-900 mb-2">Vision</div>
                <p className="text-purple-700 font-medium">To be the leading platform for legal document understanding in India</p>
              </div>
            </div>

            {/* Trust Card */}
            <div className="glass p-8 rounded-3xl bg-gradient-to-br from-green-50 to-sky-100 border border-green-200 hover:shadow-lg transition-all duration-300">
              <div className="flex items-center justify-center w-16 h-16 bg-green-500 rounded-2xl mb-6 mx-auto">
                <Shield className="h-8 w-8 text-white" />
              </div>
              <div className="text-center">
                <div className="text-4xl font-bold text-green-900 mb-2">Trusted</div>
                <p className="text-green-700 font-medium">We are trusted by the legal community</p>
              </div>
            </div>

            {/* Innovation Card */}
            <div className="glass p-8 rounded-3xl bg-gradient-to-br from-orange-50 to-rose-100 border border-orange-200 hover:shadow-lg transition-all duration-300">
              <div className="flex items-center justify-center w-16 h-16 bg-orange-500 rounded-2xl mb-6 mx-auto">
                <Clock className="h-8 w-8 text-white" />
              </div>
              <div className="text-center">
                <div className="text-4xl font-bold text-orange-900 mb-2">Innovation</div>
                <p className="text-orange-700 font-medium">We are committed to continuous innovation</p>
              </div>
            </div>
          </motion.div>
        </div>
      </div>
    </section>
  )
}
